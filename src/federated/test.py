...
x_eval = data["x_eval"]
y_eval = data["y_eval"]
record_path = 'client/results_demos/out_csv/{}'.format(CUR_ADV)
# record_path = 'client/results_demos/out_csv/fl'
create_folder(record_path)
write_pre(y_eval, os.path.join(record_path, 'truth.csv'))
x_shape = x_eval.shape
rounds = range(150, 310, 10)
csv_path = os.path.join("client/results_demos/", 'rand_adv_test.csv')
advs = ['deepfool', 'fgsm', 'pgd']
# advs = ['pgd']
model_base = "client/results_demos/models/"
outputs = []
...
for round_number in rounds:
    current_model = 'demos-adv-{}'.format(CUR_ADV)+ '-vgg16-' + '{}rounds-68clients-1epochs'.format(round_number)
    # current_model = 'demos-vgg16-' + '{}rounds-68clients-1epochs'.format(round_number)
    create_folder(os.path.join(record_path, current_model))
    MODEL_PATH = model_base + current_model
